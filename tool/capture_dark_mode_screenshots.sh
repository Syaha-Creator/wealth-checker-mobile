...
TEST="$ROOT/integration_test/dark_mode_screenshot_test.dart"
SIGNAL_PREFIX="@@SCREENSHOT@@:"

mkdir -p "$OUT"

FIFO="$(mktemp -u "${TMPDIR:-/tmp}/wc_dark_mode_screenshot.XXXXXX")"
mkfifo "$FIFO"
trap 'rm -f "$FIFO"' EXIT
...
echo "Dark mode screenshots saved to $OUT"
