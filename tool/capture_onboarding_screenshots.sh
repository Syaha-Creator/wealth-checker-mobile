...
TEST="$ROOT/integration_test/onboarding_pages_screenshot_test.dart"
SIGNAL_PREFIX="@@SCREENSHOT@@:"

mkdir -p "$OUT"

FIFO="$(mktemp -u "${TMPDIR:-/tmp}/wc_onboarding_screenshot.XXXXXX")"
mkfifo "$FIFO"
trap 'rm -f "$FIFO"' EXIT
...
echo "Onboarding screenshots saved to $OUT"
