...
TEST="$ROOT/integration_test/snackbar_feedback_screenshot_test.dart"
SIGNAL_PREFIX="@@SCREENSHOT@@:"

mkdir -p "$OUT"

FIFO="$(mktemp -u "${TMPDIR:-/tmp}/wc_snackbar_screenshot.XXXXXX")"
mkfifo "$FIFO"
trap 'rm -f "$FIFO"' EXIT
...
echo "Snackbar screenshots saved to $OUT"
